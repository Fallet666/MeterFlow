...
export function ReadingsPage({ selectedProperty }: Props) {
  ...
  if (!selectedProperty) return <div className="page">Сначала выберите объект.</div>;
  ...
  return (
    <div className="page">
      <div className="page-header">
        <div>
          <h1>Показания</h1>
          <div className="muted">Фиксируйте новые значения для расчёта начислений</div>
        </div>
      </div>

      <div className="card">
        <h3>Новое показание</h3>
        <form onSubmit={addReading} className="inline-form">
          <label>
            Счётчик
